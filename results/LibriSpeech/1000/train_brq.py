...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        feats = self.hparams.normalize(feats, wav_lens, epoch=10) # don't change normalizer anymore

        #### convolutions
        feats = self.modules.weighted_ssl_model[0](feats)
    ...
    def init_optimizers(self):
        ...
        self.weights_optimizer = self.hparams.weights_opt_class(
            self.modules.weighted_ssl_model.parameters()
        )
    ...
    #             self.modules.weighted_ssl_model.parameters()
    #         )
    #         self.optimizers_dict["weights_optimizer"] = self.weights_optimizer
    #         if self.checkpointer is not None:
    #             self.checkpointer.add_recoverable(
    #                 "weights_opt", self.weights_optimizer
    #             )

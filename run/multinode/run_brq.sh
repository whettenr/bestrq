...
python -m torch.distributed.run --nproc_per_node=4 --nnodes=${SLURM_JOB_NUM_NODES} --node_rank=${SLURM_NODEID} --master_addr=${MASTER} --master_port=5555 train.py hparams/best_rq_base.yaml \
    --find_unused_parameters --bfloat16_mix_prec --output_folder results/brq-a100 --grad_accumulation_factor=1
